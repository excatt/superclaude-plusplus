...
- **README 전면 재작성** (726 → 417줄) — v0.x→v2.0→v3.0 변천사 중심 구조를 "현재 상태 우선"으로 바꿈: 왜 이 프레임워크인가(남긴 것 3종) → 빠른 시작(설치·병합 규칙·플러그인) → 작업 흐름(난이도 분기 다이어그램) → 구성 요소(문서 5·스킬 60·에이전트 9·훅 16을 전부 표로, 훅 표는 스크립트 단위) → 핵심 개념 → 커스터마이즈 → 검증 → 디렉토리 → 요구사항 → 업데이트/삭제 → 버전 히스토리(5행 요약, 상세는 CHANGELOG) → 출처(NOTICE.md를 단일 원천으로 두고 표로 요약). What's New 3개 절과 "참고 출처 (v2.0)" 절은 CHANGELOG/NOTICE로 이관. CI 배지 추가.
...
- **`/learn` 저장 기준 강화** — Anthropic `claude-md-management` 플러그인(`update-guidelines.md`, Apache 2.0)의 "넣을 것 / 넣지 말 것" 기준을 각색해 반영. 게이트 질문("새 세션이 이걸 읽어서 실제로 시간을 아끼는가"), Include에 명령/설정 특이점/모듈 관계/검증된 테스트 접근을, Exclude에 코드에서 자명한 정보·범용 조언·일회성 수정·장황한 설명·CLAUDE.md/RULES.md 중복을 추가. 저장 위치 분기표(learned 스킬 / 프로젝트 CLAUDE.md·CLAUDE.local.md / `/note`)와 Pre-save Checklist 신설. 플러그인 자체는 미채택 — `.claude.local.md`(정식 이름은 `CLAUDE.local.md`) 탐색 오류, `@import` 미인식으로 분할 CLAUDE.md 채점 왜곡, 커맨드 `allowed-tools`와 본문 `find` 불일치, "문서화된 명령을 실제 실행해 검증" 지시의 위험, 아키텍처 20점 가점이 PRINCIPLES.md "Self-documenting Repo"와 충돌.
